...
#line 8 "Reversals_and_Sums.cpp"

using ll = long long;

struct F {};

using Node = node_t<ll, ll, F>;
ll op(ll l, ll key, ll r) { return l + key + r; }
pair<ll, ll> e() { return {0, 0}; }
pair<ll, ll> mapping(F f, Node *node) { return {node->key, node->data}; }
...
void solve(int ith) {
  int n, q;
  cin >> n >> q;
  splay_tree<ll, ll, op, e, F, mapping, composition, id> st;
  for (int i = 0; i < n; ++i) {
    int x;
    cin >> x;
    st.insert(i, x);
